...
from .evaluation import grouped_bootstrap_ci, infer_cases, metric_summary, write_evaluation_outputs
...
def run_evaluation(manifest: str, model: str, output: str, *, locked: bool=False, development_manifest: str|None=None, calibration_manifest: str|None=None, bootstrap_iterations: int=1000) -> dict:
    ...
    expected_pipeline = "phase1-0.1.0"
    ...
    try:
        ...
        installed_protocol = ReferralEngine().get_protocol_version()
    ...
    provenance={"evaluation_version":"phase3-0.1.0","test_manifest_hash":audit.manifest_hash,**partition_hashes,"model_hash":bundle.model_hash,"pipeline_version":bundle.pipeline_version,"model_version":bundle.model_version,"feature_schema_hash":bundle.feature_schema_hash,"threshold":bundle.threshold,"referral_protocol_version":installed_protocol,"executed_at":datetime.now(timezone.utc).isoformat(),"patient_leakage_check":"passed","locked_test_evaluation":locked,"image_paths_in_output":False,"telemetry":False}
    paths=write_evaluation_outputs(output,cases=cases,metrics=metrics,confidence_intervals=intervals,provenance=provenance)
    if locked:
